...
async fn main() {
    ...
    let mut dispatcher = DatagramDispatcher::bind("0.0.0.0:1234", Default::default())
        .await
        .unwrap();
    ...
    while received < MESSAGES {
        ...
        if last_print.elapsed().as_secs_f64() >= 1. {
            ...
            println!(
                "Received {} / {} ({} / {} instant, {:.02}% packet loss). Max latency: {:?}",
                received,
                total,
                instant_received,
                instant_total,
                (instant_total as f64 / instant_received as f64 - 1.) * 100.,
                max_latency,
            );
